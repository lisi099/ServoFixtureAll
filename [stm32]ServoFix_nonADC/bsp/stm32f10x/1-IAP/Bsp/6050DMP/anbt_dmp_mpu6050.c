...
//
long  dmp_quat[4];
short dmp_gyro[3], dmp_accel[3], dmp_sensors;
unsigned char dmp_more;
unsigned long dmp_sensor_timestamp;

long gyro_test[3], accel_test[3];
...
//        accel[1] =-10600448;
//        accel[2] =75655850;
        accel_test[0] = accel[0];
        accel_test[1] = accel[1];
        accel_test[2] = accel[2];
...
u8 AnBT_DMP_MPU6050_DEV_CFG(void)		
{
    ...
    if (mpu_set_gyro_fsr(1000)) return 0x10;
    if (mpu_set_accel_fsr(8)) return 0x11;   
    if (mpu_set_lpf(188)) return 0x12; 
    if (mpu_set_sample_rate(200)) return 0x13;
...
volatile float q00, q11, q22, q33;
volatile float pitch, roll, yaw;


void dmp_read_all_data(void)	//dmp读取函数
{
	if(dmp_read_fifo(dmp_gyro, dmp_accel, dmp_quat, &dmp_sensor_timestamp, &dmp_sensors, &dmp_more) == 0){
    q00 = dmp_quat[0] / Q30;
    q11 = dmp_quat[1] / Q30;
    q22 = dmp_quat[2] / Q30;
    q33 = dmp_quat[3] / Q30;
    pitch = asin(-2 * q11 * q33 + 2 * q00* q22)* 57.3f;                                       // pitch
    roll  = atan2(2 * q22 * q33 + 2 * q00 * q11, -2 * q11 * q11 - 2 * q22* q22 + 1)* 57.3f;   // roll
    yaw   = atan2(2*(q11*q22 + q00*q33),q00*q00+q11*q11-q22*q22-q33*q33) * 57.3f;	            // yaw
  }    
}
