...
extern uint8_t menu_combine_fb_work_parm(void);
extern void read_work_param(uint8_t seq);
static void tai_servo(void* parameter)
{
	...
	PWM_HIGH1;
	uint8_t data[2] = {0x10, 0xF0};
	usart3_init();
	adc_configration();
	rt_thread_delay(10);
	while(1)
	{
//		menu_combine_fb_work_parm();
//		read_work_param(1);
		rt_thread_delay(500);
...
//    if(tid1 != RT_NULL) rt_thread_startup(tid1);
	...
	tid1 = rt_thread_create("servo_tai", tai_servo, RT_NULL, 1024, 20, 3);
    if(tid1 != RT_NULL) rt_thread_startup(tid1);
