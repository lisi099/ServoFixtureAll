...
public:
    lcd_protocol(QSerialPort *port):serial_(port)
    {
        ...
        int i =0;
        version_map_index_.insert(40, i);
        ++i;
        version_map_index_.insert(41, i);
        ++i;
        version_map_index_.insert(1, i);
        ++i;
        version_map_index_.insert(2, i);
        ++i;
        version_map_index_.insert(3, i);
        ++i;
        version_map_index_.insert(4, i);
        ++i;
        version_map_index_.insert(5, i);
        ++i;
        version_map_index_.insert(6, i);
        ++i;
        version_map_index_.insert(7, i);
        ++i;
        version_map_index_.insert(10, i);
        ++i;
        version_map_index_.insert(11, i);
        ++i;
        version_map_index_.insert(8, i);
        ++i;
        version_map_index_.insert(80, i);
        ++i;
        version_map_index_.insert(81, i);
        ++i;
        version_map_index_.insert(82, i);
        ++i;
        version_map_index_.insert(83, i);
        ++i;
        version_map_index_.insert(84, i);
        is_tai_servo_ = false;
    }
...
//        delete buff;
    ...
    void get_version(Servo_Tai_Data_ &data)
    {
        uint8_t buf_2[2], buf[2];
        buf_2[0] = data.version / 1000 % 10;
        buf_2[1] = data.version / 100 % 10;
        uint16_t distribtor = buf_2[0] * 10 + buf_2[1];
        if(!version_map_.contains(distribtor)){
            version_ = "XX-XX-XX";
            qDebug() << "not find version" << distribtor;
            return;
        }

        buf[0] =  data.version / 10 % 10;
        buf[1] =  data.version / 1 % 10;
        uint16_t costormer = buf[0] * 10 + buf[1];
        ...
        uint8_t factory = data.version / 10000 % 10 + 1;
        version_ = QString("v1.%1 %2-%3").arg(factory).arg(version_map_.find(distribtor).value()).arg(costormer_str);
        qDebug() << version_;
    }
    ...
    void get_data(ui_data &data)
    {
        if(is_tai_servo_ == false){
            ...
            data.version = servo_tai_data_.version;
            data.max_power =(servo_tai_data_.max_power - 50)/5;
            data.dead_band = servo_tai_data_.deadband;
            data.brake = (servo_tai_data_.brake - 20)/10;
            data.center_num = (servo_tai_data_.senter -2048)/28;
            qDebug() << "soft_start" <<servo_tai_data_.soft_start;
            if(servo_tai_data_.soft_start!= 0){
                data.soft_start = 1;
            }
            else{
                data.soft_start = 0;
            ...
            //force
            if(servo_tai_data_.farce <=90){
                data.force = 1;
            }
            else if(servo_tai_data_.farce <=100){
                data.force = 2;
            }
            else if(servo_tai_data_.farce <=110){
                data.force = 3;
            }
            else if(servo_tai_data_.farce <=120){
                data.force = 4;
            }
            else if(servo_tai_data_.farce <=130){
                data.force = 5;
            }
            else if(servo_tai_data_.farce <=155){
                data.force = 6;
            }
            else if(servo_tai_data_.farce <=165){
                data.force = 7;
            }
            else if(servo_tai_data_.farce <=175){
                data.force = 8;
            }
            ...
            //tension
            if(servo_tai_data_.tension <=130){
                 data.tension = 1;
            }
            else if(servo_tai_data_.tension <=200){
                data.tension = 2;
            }
            else if(servo_tai_data_.tension <=270){
                data.tension = 3;
            }

            //boost
            if(servo_tai_data_.boost >=1300){
                data.boost  = 1;
            }
            else if(servo_tai_data_.boost >=1200){
                data.boost  = 2;
            }
            else if(servo_tai_data_.boost >=1100){
                data.boost  = 3;
            }
            else if(servo_tai_data_.boost >=1000){
                data.boost  = 4;
            }
            else if(servo_tai_data_.boost >=900){
                data.boost  = 5;
            }
            else if(servo_tai_data_.boost >=800){
                data.boost  = 6;
            }
            else if(servo_tai_data_.boost >=700){
                data.boost  = 7;
            }
            else if(servo_tai_data_.boost >=600){
                data.boost  = 8;
            }
            else if(servo_tai_data_.boost >=500){
                data.boost  = 9;
            }
            else if(servo_tai_data_.boost >=400){
                data.boost  = 10;
            }
...
public:
    ...
    Servo_Tai_Data_ servo_tai_data_;
    QString version_;
    bool is_tai_servo_;
